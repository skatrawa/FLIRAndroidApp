...
public class GLPreviewActivity extends Activity implements Device.Delegate, FrameProcessor.Delegate, Device.StreamDelegate, Device.PowerUpdateDelegate{
    ...
    private volatile boolean begintrace= false;
    ...
    private FileOutputStream logdatahandle;
    //private volatile boolean waitbit;
    ...
    public void onFrameProcessed(final RenderedImage renderedImage){
        if (renderedImage.imageType() == RenderedImage.ImageType.ThermalRadiometricKelvinImage){
            ...
            this.end = this.end / 1000000000;
            NumberFormat numberFormat = NumberFormat.getInstance();
            numberFormat.setMaximumFractionDigits(2);
            numberFormat.setMinimumFractionDigits(2);
            final String timestamp = numberFormat.format(this.end);
            final String spotMeterValue = numberFormat.format(averageC) + "ºC";
            logdatastr = logdatastr +  timestamp + "," + numberFormat.format(averageC) + "\n";
        ...
        /*if(waitbit)
        {
           try{
               wait(1000);
               logdatastr = logdatastr  + "STOP" + "\n";
           }
           catch(Exception e){
               Log.v("WAIT EXCEPTION","Exception" + e);
           }
        }*/
    ...
    public void onTraceClicked (View v) {
        ToggleButton theSwitch = (ToggleButton) v;
        if (theSwitch.isChecked()) {
            this.begintrace = true;
           // waitbit = true;
            logdatastr = "";
            Log.v("Trace Clicked", "Status=ON");
            this.start = System.nanoTime();
            String path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).toString();
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd-HH-mm-ssZ", Locale.getDefault());
            String formatedDate = sdf.format(new Date());
            logdatapath = path + "/" + "FLIROne-" + formatedDate + ".csv";
            Log.v("Trace Clicked",logdatapath);
            try {
                File newfile = new File(logdatapath);
                newfile.createNewFile();
                logdatahandle = new FileOutputStream(newfile);
            } catch (Exception e) {
                Log.v("Trace Clicked", "Failed to create the file path; Switch toggled back");
                theSwitch.setChecked(false);
                this.begintrace = false;
             //   waitbit = false;
                 Log.v("Trace Clicked","Exception:"+e);
            }
        } else {
            this.begintrace = false;
          //  waitbit = false;
            byte[] logdatabyte = logdatastr.getBytes();
            Log.v("Trace Clicked", "Status=OFF");
            try {
                logdatahandle.write(logdatabyte);
                logdatahandle.close();
            } catch (Exception e) {
                Log.v("OnTraceClicked", "Failed to write to the file");
            }
        }
    }
